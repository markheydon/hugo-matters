...
public sealed class SaveEndpointTests
{
    ...
    public async Task Save_ReturnsCommitSha_AndClearsUnsavedEdits()
    {
        ...
        var saveResult = await saveResponse.Content.ReadFromJsonAsync<SaveResult>(HugoMatterApiFactory.JsonOptions);
        ...
        var session = await sessionResponse.Content.ReadFromJsonAsync<EditingSession>(HugoMatterApiFactory.JsonOptions);
        Assert.NotNull(session);
        Assert.False(session.HasUnsavedLocalEdits);
    }

    private static void ConfigureGitHub(HugoMatterApiFactory factory)
    {
